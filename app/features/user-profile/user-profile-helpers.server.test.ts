...
describe('throwIfUserProfileIsMissing()', () => {
  test('given a user profile (the user exists): returns the user', () => {
    ...
    expect(throwIfUserProfileIsMissing(generateRandomDid())(user)).toEqual(user)
  })
  ...
  test("given null (the user doesn't exist): throws the correct error", () => {
    ...
    expect(() => throwIfUserProfileIsMissing(userId)(user)).toThrowError('No user profile found with id: ' + userId)
  })
})
