...
describe('UserProfile component', () => {
  it("given a user's avatar, email and name and no success: renders the correct headings and the user's avatar, email and name and hide the success banner", async () => {
    ...
    const RemixStub = createRemixStub([{ path, element: <UserProfileComponent {...props} /> }])
    ...
    expect(screen.getByText(/information will be displayed publicly/i)).toBeInTheDocument()
  ...
  it('given a success: renders the success banner', () => {
    ...
    expect(screen.getByRole('link', { name: /dismiss/i })).toHaveAttribute('href', '/settings/profile')
  })
})
