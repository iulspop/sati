...
describe.skip('UserAuthentication component', () => {
  it('given an idle state and nothing else: renders the user authentication form', async () => {
    const user = userEvent.setup()
    const path = '/login'
    const props = createProps()
    const RemixStub = createRemixStub([{ path, element: <UserAuthenticationComponent {...props} /> }])

    render(<RemixStub initialEntries={[path]} />)

    // It renders the correct headings and a sub heading.
    expect(screen.getByRole('heading', { name: /sign in or sign up/i, level: 1 })).toBeInTheDocument()
    expect(
    ...
    expect(screen.getByText(/or create an account/i, { selector: 'p' })).toBeInTheDocument()
  ...
  it('given an email and an email error: displays the error for the email field', () => {
    ...
    expect(screen.getByRole('textbox', { description: emailError })).toHaveValue(email)
    expect(screen.getByRole('alert')).toHaveTextContent(emailError)
    expect(screen.getByRole('textbox', { name: /email address/i })).not.toBeValid()
  })
