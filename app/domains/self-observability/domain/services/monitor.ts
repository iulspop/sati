...
const loadSLOandResults = (SLOs: SLOsAPI, Streams: StreamsAPI) =>
  asyncPipe(
    ...
    async ({ slo, stream }) => ({
      slo,
      results: interpret(await Streams.readEvents(stream.id)),
    })
  )
...
export const currentPercentage: CurrentPercentage = ({ slo: { denominator }, results }) =>
  toSecondDecimal(results.filter(Boolean).length / denominator)
...
const toSecondDecimal = (number_: number): number => Math.floor(number_ * 100) / 100
