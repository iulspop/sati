...
import { getSafeRedirectDestination, isValidRedirectDestination } from './get-safe-redirect-destination.server'
...
describe('getSafeRedirectDestination()', () => {
  test('given a request with a url: returns the safe redirect destination', () => {
    ...
    const request = new Request(`http://localhost:3000?redirectTo=${destination}`)
