...
type AsyncPipeReturnType<FS extends Callback[], P = Parameters<FS[0]>[0]> = FS extends [...infer _, infer Last]
  ? (a: P) => Promise<FunToReturnType<Last>>
  : EmptyPipe

type AsyncParameters<FS extends Callback[], P = Parameters<FS[0]>[0]> = FS extends [infer H, ...infer Rest]
...
export function asyncPipe<A, B, C>(ab: (a: A) => MaybePromise<B>, bc: (b: B) => MaybePromise<C>): (a: A) => Promise<C>
...
export function asyncPipe(...fns: AsyncParameters<any[]>) {
  ...
  return (x: Parameters<(typeof fns)[0]>[0]) => fns.reduce(async (y, function_) => function_(await y), x)
}
